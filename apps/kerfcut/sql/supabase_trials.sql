...
BEGIN
    INSERT INTO public.trials (machine_id, runs_count)
    VALUES (p_machine_id, 1)
    ON CONFLICT (machine_id) DO UPDATE
    SET runs_count = public.trials.runs_count + 1
    RETURNING public.trials.runs_count, public.trials.started_at
    INTO v_runs_count, v_started_at;

    runs_count := v_runs_count;
    days_left := GREATEST(0, 30 - FLOOR(EXTRACT(EPOCH FROM (now() - v_started_at)) / 86400)::INTEGER);
    RETURN NEXT;
END;
$$;
