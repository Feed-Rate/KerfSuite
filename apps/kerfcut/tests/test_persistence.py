...
def sample_job() -> Job:
    ...
    job.unplaced = [
        Piece(quantity=2, width=3000, height=3000, label="Too big")
    ]
    layout = SheetLayout(sheet=job.sheets[0], waste_area=100)
    layout.placed.append(
        PlacedPiece(piece=job.pieces[0], x=10, y=10, width=800, height=500, rotated=False, color=(255, 0, 0))
    )
...
def test_save_and_load_roundtrip():
    ...
    assert len(loaded.unplaced)   == len(job.unplaced)
    assert len(loaded.layouts)    == len(job.layouts)

    l0 = loaded.layouts[0]
    assert l0.sheet.width == job.layouts[0].sheet.width
    assert l0.waste_area == 100
    assert len(l0.placed) == 1
    assert l0.placed[0].piece.width == 800
    assert l0.placed[0].x == 10
    assert l0.placed[0].color == (255, 0, 0)
...
def test_missing_ids_are_regenerated_on_load():
    data = {
        "name": "Legacy",
        "sheets": [{"width": 1000, "height": 500}],
        "pieces": [{"quantity": 1, "width": 200, "height": 100}],
    }
    job = job_from_dict(data)
    assert job.sheets[0].id
    assert job.pieces[0].id


def test_duplicate_ids_are_regenerated_on_load():
    data = {
        "name": "Duplicated IDs",
        "sheets": [
            {"id": "same-sheet", "width": 1000, "height": 1000},
            {"id": "same-sheet", "width": 500, "height": 500},
        ],
        "pieces": [
            {"id": "same-piece", "quantity": 1, "width": 900, "height": 900},
            {"id": "same-piece", "quantity": 1, "width": 450, "height": 450},
        ],
    }

    job = job_from_dict(data)

    assert len({s.id for s in job.sheets}) == 2
    assert len({p.id for p in job.pieces}) == 2
