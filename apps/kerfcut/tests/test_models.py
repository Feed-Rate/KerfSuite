...
from core.models import (
    Job, Sheet, Piece, SheetLayout, PlacedPiece,
    ensure_unique_job_ids, sheet_report_key, piece_report_key,
)
...
def test_ensure_unique_job_ids_regenerates_duplicates():
    sheet_id = "same-sheet"
    piece_id = "same-piece"
    job = Job(
        sheets=[
            Sheet(id=sheet_id, width=1000, height=1000),
            Sheet(id=sheet_id, width=500, height=500),
        ],
        pieces=[
            Piece(id=piece_id, quantity=1, width=900, height=900),
            Piece(id=piece_id, quantity=1, width=450, height=450),
        ],
    )
...
def test_report_keys_separate_duplicate_ids_with_different_details():
    sheet_id = "same-sheet"
    piece_id = "same-piece"

    large_sheet = Sheet(id=sheet_id, width=1000, height=1000)
    small_sheet = Sheet(id=sheet_id, width=500, height=500)
    large_piece = Piece(id=piece_id, width=900, height=900)
    small_piece = Piece(id=piece_id, width=450, height=450)

    assert sheet_report_key(large_sheet) != sheet_report_key(small_sheet)
    assert piece_report_key(large_piece) != piece_report_key(small_piece)
