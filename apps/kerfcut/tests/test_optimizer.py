...
def test_duplicate_sheet_ids_do_not_collapse_sheet_rows():
    sheet_id = "same-sheet"
    job = make_job(
        sheets=[
            Sheet(id=sheet_id, width=1000, height=1000, active=True, quantity=1, label="Big"),
            Sheet(id=sheet_id, width=500, height=500, active=True, quantity=1, label="Small"),
        ],
        pieces=[Piece(quantity=2, width=450, height=450)],
    )

    result = optimize(job)

    assert result.total_pieces_placed == 2
    assert len({s.id for s in result.sheets}) == 2
    assert sorted(layout.sheet.label for layout in result.layouts) == ["Big", "Small"]


def test_duplicate_piece_ids_do_not_merge_unrelated_unplaced_rows():
    piece_id = "same-piece"
    job = make_job(
        sheets=[Sheet(width=500, height=500, active=True, quantity=1)],
        pieces=[
            Piece(id=piece_id, quantity=1, width=900, height=900, label="Large"),
            Piece(id=piece_id, quantity=1, width=450, height=450, label="Small"),
        ],
    )
    ...
    assert result.total_pieces_placed == 1
    assert len(result.unplaced) == 1
    assert result.unplaced[0].label == "Large"
    assert len({p.id for p in result.pieces}) == 2
