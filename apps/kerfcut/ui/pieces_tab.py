...
class PiecesTab(QWidget):
    ...
    def _build(self):
        ...
        # Ctrl+V paste shortcut — scoped to the table widget so it doesn't
        # hijack paste when editing a cell (e.g. typing a label)
    ...
    def _paste_from_clipboard(self):
            ...
            if len(parts) >= 3:
                try:
                    ...
                    if qty > 0 and w > 0 and h > 0:
                        self._append_row(Piece(quantity=qty, width=w, height=h, label=label, can_rotate=True))
                        imported += 1
                    else:
                        skipped += 1
                except ValueError:
        ...
        if imported > 0 or skipped > 0:
            if imported > 0:
                self._update_total()
                self.main_win.mark_dirty()
            msg = f"Imported {imported} piece(s) from clipboard."
            if skipped > 0:
                msg += f"\nSkipped {skipped} row(s) with invalid data."
            QMessageBox.information(self, "Paste Result", msg)
    ...
    def _append_row(self, piece: Piece):
        ...
        qty_item = self._num_item(piece.quantity)
        qty_item.setData(Qt.ItemDataRole.UserRole, piece.id)
        self.table.setItem(r, COL_QTY, qty_item)
    ...
    def _read_row(self, r: int) -> Piece:
        ...
        # Preserve the original piece ID if stored, otherwise generate new
        qty_item = self.table.item(r, COL_QTY)
        piece_id = qty_item.data(Qt.ItemDataRole.UserRole) if qty_item else None
