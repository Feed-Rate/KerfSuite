...
class SheetsTab(QWidget):
    ...
    def _add_from_library(self):
        if not self._library_cache:
            QMessageBox.information(
                ...
                "Add a sheet to the job, then click '💾 Save to Library' to build your library."
            )
            return
    ...
    def _append_row(self, sheet: Sheet):
        ...
        chk.setData(Qt.ItemDataRole.UserRole, sheet.id)
    ...
    def _read_row(self, r: int) -> Sheet:
        ...
        # Preserve the original sheet ID if stored, otherwise generate new
        sheet_id = active_item.data(Qt.ItemDataRole.UserRole) if active_item else None
