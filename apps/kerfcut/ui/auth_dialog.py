...
from core.auth import (
    ...
    force_trial_mode_enabled, get_machine_id_display, get_license_info
)

class VerifyLicenseThread(QThread):
    ...
    def run(self):
        is_valid = verify_license(self.key)
...
class LicenseDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        if tier == "trial":
            sub_text = f"You are currently in a Trial ({info.get('days_left', 0)} days, {info.get('runs_left', 0)} runs left).\nEnter a license key to activate full features, or continue your trial."
        elif tier == "expired":
            sub_text = "Your trial has expired. The application is now in read-only mode.\nPlease enter a license key to restore full functionality."
        else:
            sub_text = "Please enter your license key to continue."

        self.subtitle = QLabel(sub_text)
        self.subtitle.setWordWrap(True)
        if tier == "expired":
            self.subtitle.setStyleSheet("color: #cc0000; font-weight: bold;")
        ...
        continue_text = "Continue Trial" if tier == "trial" else "Continue (Read-Only)"
        self.continue_btn = QPushButton(continue_text)
    ...
    def apply_styles(self):
        self.setStyleSheet("""
            ...
            QPushButton#quit_btn, QPushButton#continue_btn {
                background-color: #ffffff;
                color: #2c3e50;
                border: 1px solid #d1d9e6;
            }
            QPushButton#quit_btn:hover, QPushButton#continue_btn:hover {
                background-color: #eef2f7;
            }
            QMessageBox QLabel {
                color: #2c3e50;
    ...
    def _copy_machine_id(self):
        ...
        # Reset icon after 1.5 seconds safely
    ...
    def _reset_copy_btn(self):
        try:
            ...
            pass  # Dialog was closed and C++ object destroyed
    ...
    def handle_activation(self):
        ...
        self._verify_thread = VerifyLicenseThread(key, self)
        self._verify_thread.result.connect(self._on_verification_done)
        self._verify_thread.start()

    def _on_verification_done(self, is_valid: bool):
        if is_valid:
            self.accept()
        else:
