...
class MainWindow(QMainWindow):
    ...
    def mark_dirty(self):
        ...
        # Show a stale-results warning instead of destroying layouts,
        # so users can still view old results while tweaking inputs.
        if self.job.layouts:
    ...
    def _push_history(self):
        ...
        if len(self._history) > 50:
            ...
            self._history_index -= 1
            self._saved_history_index -= 1
            # If the saved state has been pushed off the buffer, mark as lost
            if self._saved_history_index < -1:
                self._saved_history_index = -2  # sentinel: saved state unreachable
