        ...
        for i, pp in enumerate(self.layout_template.placed):
            ...
            if QRect(px, py, pw, ph).contains(event.pos()):
                ...
                self.setToolTip(
                    ...
                    f"Position: ({display_x}, {display_y})\n"
...
class CutPlanTab(QWidget):
    ...
    def load_from_job(self, job):
        ...
        from core.models import group_identical_layouts, piece_report_key
        ...
        unplaced_qty = sum(p.quantity for p in job.unplaced)
        txt = (f"✅ {placed}/{needed} pieces placed on {job.sheets_used} sheet(s)  |  "
               f"Overall efficiency: {eff:.1f}%")
        if unplaced_qty > 0:
            txt += f"  ⚠️  {unplaced_qty} piece(s) unplaced — add more sheets!"
        self.stats_label.setText(txt)
        color = "#cc0000" if unplaced_qty > 0 else "#1a7a1a"
