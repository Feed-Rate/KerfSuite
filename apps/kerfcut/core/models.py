...
def ensure_unique_job_ids(job: "Job") -> None:
    """Ensure editable job rows have stable, unique IDs."""
    def normalize(items: list) -> None:
        seen = set()
        for item in items:
    ...
    normalize(job.sheets)
    normalize(job.pieces)


def sheet_report_key(sheet: Sheet) -> tuple:
    """Key used when grouping sheet usage for reports."""
    return (
        sheet.id,
        sheet.width,
        sheet.height,
...
def piece_report_key(piece: Piece) -> tuple:
    """Key used when matching pieces for cut-plan quantity reports."""
    return (
        piece.id,
        piece.width,
        piece.height,
