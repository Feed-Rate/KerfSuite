...
def add_to_library(sheet: Sheet):
    """Add a single sheet to the library, or update it if it already exists."""
    lib = get_library()
    
    sheet_label = (sheet.label or "").strip().lower()
    
    for i, existing in enumerate(lib):
        existing_label = (existing.label or "").strip().lower()
        if (existing.width == sheet.width and 
            existing.height == sheet.height and 
            existing.thickness == sheet.thickness and 
            existing_label == sheet_label):
            # Update existing entry with new properties (e.g., prices)
            lib[i] = sheet
            save_library(lib)
            return
