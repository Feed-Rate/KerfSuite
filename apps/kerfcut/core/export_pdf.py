...
def _add_page_header(story, job, normal_style):
    ...
    left_data = []
    left_data.append([Paragraph("Job:", normal_style), Paragraph(escape(job.name), normal_style)])
    if job.customer:
        left_data.append([Paragraph("Customer:", normal_style), Paragraph(escape(job.customer), normal_style)])
    if job.material_name:
        left_data.append([Paragraph("Material:", normal_style), Paragraph(escape(job.material_name), normal_style)])
    if job.notes:
        left_data.append([Paragraph("Notes:", normal_style), Paragraph(escape(job.notes), normal_style)])
...
def export_pdf(job, filepath: str, currency: str = "R") -> None:
    ...
    # PAGE 3+: CUT PLANS
    # =========================================================================
    from .models import group_identical_layouts, piece_report_key
    groups = group_identical_layouts(job.layouts)

    piece_stats = {piece_report_key(p): {"total": p.quantity, "cut": 0} for p in job.pieces}
    ...
    for idx, group in enumerate(groups):
        ...
        for pp in layout.placed:
            key = piece_report_key(pp.piece)
            if key not in pieces_on_template:
                pieces_on_template[key] = {"count": 0, "piece": pp.piece}
            pieces_on_template[key]["count"] += 1
